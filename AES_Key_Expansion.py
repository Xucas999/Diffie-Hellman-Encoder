...
def key_expansion(key):
    ...
    for i in range(4, 44):
        ...
        if i % 4 == 0:
            temp = substitution_box(rotate_bytes(temp))
            temp[0] ^= RCON[i // 4]
...
if __name__ == "__main__":
    key = b'thisis16bytekey1'
    round_keys = key_expansion(key)

    for i in range(11):
        round_key = sum(round_keys[i*4:i*4+4], [])
        print(f"Round {i}: {bytes(round_key).hex()}")